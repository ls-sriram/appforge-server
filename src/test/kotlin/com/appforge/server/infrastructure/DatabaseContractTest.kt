...
/**
 * Contract tests for the [Database] interface.
 * Run against InMemoryDatabase by default.
 * When PostgreSQL is available, run against SqlDatabase too.
 */
abstract class DatabaseContractTest {
    protected abstract fun createDatabase(): Database
    ...
    fun `update replaces document`() = runBlocking {
        ...
        assertNull(result.data["age"]) // full replace
    }
    ...
    fun `delete is idempotent`() = runBlocking {
        val db = createDatabase()
        db.delete("users", "nonexistent") // should not throw
    }
    ...
    fun `merge updates existing document preserving fields`() = runBlocking {
        ...
        val result = db.get("users", "user-1")
        assertTrue(result is Resource.Success)
        assertEquals("Alice", result.data["name"])   // preserved
        assertEquals(31.0, result.data["age"])       // updated
        assertEquals("admin", result.data["role"])    // added
    }
...
/** Run contract tests against InMemoryDatabase. */
class InMemoryDatabaseTest : DatabaseContractTest() {
    override fun createDatabase(): Database = InMemoryDatabase()
}

/** Run contract tests against ExposedDatabase with Testcontainers. */
@org.junit.jupiter.api.Disabled("Legacy document-store contract tests are obsolete after SQL-only persistence migration.")
class ExposedDatabaseContractTest : DatabaseContractTest() {
    companion object {
        private val postgres = org.testcontainers.containers.PostgreSQLContainer<Nothing>(
